...
class PartnerApp:
    ...
    def __init__(self, root, load_partners=None, preview_partners=None):
        self.root = root
        self.load_partners = load_partners
        self.preview_partners = preview_partners
        self.password = None
        self.loading = False
        self.closed = False
        ...
        self.logo_image = tk.PhotoImage(file=str(resources / "company_logo.png"))
        self.icon_image = tk.PhotoImage(file=str(resources / "app_icon.png"))
        ...
        if preview_partners is not None:
            self.partners = preview_partners
            self.render_partners()
            self.connection_button.configure(state="disabled")
        else:
            self.partners = []
            self.show_message("Подключитесь, чтобы увидеть партнеров компании.")
    ...
    def build_layout(self):
        ...
        self.status = tk.StringVar(value="Нет подключения")
    ...
    def render_partners(self):
        if self.loading:
            return
        if self.password is None and self.preview_partners is None:
            return
        ...
        prefix = "Предпросмотр · пример данных" if self.preview_partners is not None else "Данные загружены"
        self.status.set(f"{prefix}  |  Показано: {len(partners)} из {len(self.partners)}")
    ...
    def add_card(self, partner):
        ...
        self.make_label(card, format_rating(partner.get("rating"))).grid(
            row=3, column=0, sticky="w", padx=22, pady=(0, 14),
        )
        self.cards.append(card)

    def connect(self):
        if self.loading or self.preview_partners is not None:
            return
    ...
    def refresh(self):
        if self.loading:
            return
        if self.preview_partners is not None:
            self.render_partners()
            return
    ...
    def poll_results(self):
        ...
        try:
            ...
            if success:
                ...
                self.partners = []
                self.password = None
                self.show_message("Не удалось загрузить партнеров. Проверьте подключение и повторите вход.")
